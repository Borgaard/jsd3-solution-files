...
function makeDataFetcher(callback, type) {
  return function(url) {
    $.ajax({
        url: url,
...
var getWeather = makeDataFetcher(updateWeather, weatherData.category);
var getTime = makeDataFetcher(updateTime, timeData.category);

getWeather(weatherData.url);
getTime(timeData.url);
